...
#   - Automatically creates .isle domain mappings
...
Description:
  ...
  This service discovers devices via mDNS and creates .isle domain mappings.
...
# Discovers .local domains via mDNS and creates .isle mappings
...
# Discovers mDNS .local domains and creates .isle DNS entries
...
# Isle Mesh .isle Domain Mappings
...
EOF
    ...
    echo -e "$domains" | while IFS='|' read -r hostname ip; do
        ...
        # Create both .local and .isle mappings pointing to same IP
        echo "address=/${hostname}.local/${ip}" >> "$tmpconf"
        echo "address=/${hostname}.isle/${ip}" >> "$tmpconf"

        log_info "Mapped: ${hostname}.local -> ${hostname}.isle (${ip})"
    done
...
${BLUE}What It Does:${NC}
  ...
  • Enables access via both .local and .isle domains
...
${BLUE}Testing:${NC}
  ...
  3. Check DNS: ${CYAN}nslookup hostname.isle ${ROUTER_IP}${NC}
  4. Test access: ${CYAN}curl http://hostname.isle${NC}

${BLUE}Next Steps:${NC}
  1. Ensure isle-agents are advertising mDNS
  2. Configure agents to respond to .isle domains
